                ...
                if (wParam == DBT_DEVICEARRIVAL) {
                    foreach (const QString &drive, drives) {
                        ...
                        if (db_volume->dbcv_flags & DBTF_MEDIA)
                            qWarning("Drive %c: Media has been arrived.", drive.at(0).toLatin1());
                        else if (db_volume->dbcv_flags & DBTF_NET)
                            qWarning("Drive %c: Network share has been mounted.", drive.at(0).toLatin1());
                        else
                            qWarning("Drive %c: Device has been added.", drive.at(0).toLatin1());
#endif
